...
description: Generate an on-brand image/asset for Built for Exit. Reads the brand's brand.txt, uses the best available image model, passes the Golden Atomic Brand References (the Hollow 4 mark plus the data-graphic + founder-photography set), honors the banned terms, and stamps the real wordmark. Hosted by the Built for Exit brand OS as the canonical generation procedure, always current.
---
