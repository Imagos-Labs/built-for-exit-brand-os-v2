---
name: create-gabr
description: Create a NEW Golden Atomic Brand Reference (GABR) for Built for Exit, a permanent reference render that joins the canon, not a throwaway asset. Builds on create-on-brand-image, adds the GABR discipline: fit the dark / Exit-Yellow / engineered universe (founder-photography + vector data graphics, the Hollow 4), cohere with the existing GABRs, validate with a human, store the prompt + reference images + alt text, register it, and regenerate brand.txt. Hosted by the Built for Exit brand OS, always current.
---
...
2. **Generate for coherence.** Run create-on-brand-image, but **pass the nearest existing GABRs as input references** so the new render inherits the universe (dark ground, the one Exit-Yellow accent, the engineered finish, recurring figures). Coherence with the set is the job, not novelty. Vector GABRs (diagrams/scorecards) are authored as SVG in `../../motifs/`, not generated.
...
Before you register it, put the new render next to the existing GABRs. It passes only if a stranger would say they belong to the same universe — same dark ground, palette, single accent, lighting, and finish. If it reads as an outlier, fix it or drop it.
